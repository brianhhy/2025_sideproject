...
const Document = ({ menuItems, setMenuItems, fetchMenuItems }) => {
  ...
  const handleFileChange = (event) => {
    ...
    const fileUpload = async (e) => {
      ...
      try {
        const resp = await AUTH_API.post("http://localhost:8080/api/user/naver-cloud/stt", formData, {
          headers: {
    ...
    if (resp.data.text) {
      setSections((prevSections) => {
            const lastIndex = prevSections.length - 1;
            const updatedSections = [...prevSections];

            // 기존 마지막 섹션 뒤에 변환된 텍스트 추가
            updatedSections[lastIndex] += ` ${resp.data.text}`;
            return updatedSections;
            });
        }
        // ✅ 서버 응답 처리
        console.log(`서버응답 : ${resp.data.text}`);
      } catch (error) {
      ...
      <div className="flex w-full h-screen bg-gray-200 relative">
        ...
        >
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center space-x-4">
         {/* 녹음 시작 버튼 */}
              <button onClick={handleStartRecording} className="w-10 h-10 bg-red-500 rounded-full flex items-center justify-center">
              ...
              {/*녹음 중지 버튼*/}
              <button onClick={handleStopRecording} className="w-10 h-10 bg-gray-500 rounded-full flex items-center justify-center">
                <StopIcon style={{color: "white", fontSize: "24px"}}/>
              </button>
            </div>

          {/* 🎯 `mediaBlobUrl`을 기반으로 오디오 제공*/}
          {mediaBlobUrl && (
            <>
              <audio ref={audioRef} src={mediaBlobUrl} controls className="mt-2 w-full"></audio>
              <a href={mediaBlobUrl} download="recorded-audio.wav" className="block text-center text-blue-500 mt-2">
                📥 다운로드
              </a>
            </>
          )}
         ...
         {/* 파일 선택 및 업로드 */}
         <div className="mb-6">
          <h3 className="text-sm font-bold border-b border-gray-300 pb-1 mb-2">📤 파일 선택</h3>
          <input type="file" onChange={handleFileChange} accept="audio/*" className="border border-gray-300 p-1 rounded-md w-full mb-2" />
          <button onClick={fileUpload} className="w-full bg-blue-500 text-white p-2 rounded-md">
            Text 변환
          </button>
          {<div className="mt-2 text-center font-semibold text-blue-700"></div>}
        </div>
