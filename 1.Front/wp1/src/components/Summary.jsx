...
const Summary = () => {
  ...
  const handleContentChange = (index, event) => {
    ...
    if (index === sections.length - 1 && event.target.scrollHeight > event.target.offsetHeight) {
  ...
  const handleCreateQuiz = async () => {
    if (sections) {
      localStorage.setItem("QuizContent",sections);
      navigate("/memo/docs/summary/questions");
    }
    else{
      console.log("summary sections가 존재하지 않습니다.")
    }
  }
  ...
  return (
      <div className="flex w-full h-screen bg-gray-200 relative">
        ...
        {/* Right Sidebar */}
        <div className="fixed bottom-0 right-4 w-[calc(20%-10px)] h-[80%] bg-white border-l border-gray-300 p-4 flex flex-col rounded-tl-lg shadow-lg">
          {/* Text Section */}
          <div className="mb-6">
